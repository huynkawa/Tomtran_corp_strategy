...
from src.ui_streamlit_layout import render_ui
from src.chat_saver import save_chat

# 🚀 Tạo UI và lấy các tham số
ui_vals = render_ui("TOMTRANCHATBOT")

# Nếu UI chưa sẵn sàng hoặc trả về None → dừng an toàn
if not ui_vals:
    st.stop()

# Đảm bảo đúng kiểu và đủ 9 giá trị
if not isinstance(ui_vals, (tuple, list)) or len(ui_vals) != 9:
    st.error("render_ui() phải trả về 9 giá trị: user_msg, temperature, top_p, fallback_general, K, MIN_RELEVANCE, debug_mode, show_system, show_rag.")
    st.stop()
...
current_chat = st.session_state.get("current_chat", "Đoạn Chat 1")
...
# --- FOOTER CHUYÊN NGHIỆP (hiển thị cuối giao diện) ---
st.markdown("""
    <hr style="margin-top:2rem; border: 0.5px solid #333;">
    <div style="text-align:center; color:#888; font-size:0.9rem; line-height:1.6; margin-top:0.5rem;">
        <span style="font-weight:600; color:#ccc;">🤖 TOMTRAN Chatbot</span> — 
        <span style="color:#aaa;">The best assistant designed for strategic insight, risk analysis, and business intelligence.</span><br>
        <span style="font-size:0.85rem; color:#666;">© 2025 Tom Tran Corporation. All rights reserved.</span>
    </div>
