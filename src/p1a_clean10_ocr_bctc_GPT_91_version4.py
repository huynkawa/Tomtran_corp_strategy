...
OUTPUT_ROOT_DEFAULT = r"D:\1.TLAT\3. ChatBot_project\1_Insurance_Strategy\outputs\p1a_clean10_ocr_bctc_GPT_91_version4"
...
# [ADD] — nhận diện tail numeric columns trên 1 hàng
def _tail_numeric_span(cells: List[str]) -> Tuple[int,int]:
    """
    Trả về (start_idx, count) của cụm cột số liên tiếp ở rìa phải.
    Nếu không có, trả (len(cells), 0).
    """
    k = len(cells)
    if k == 0: return (0, 0)
    j = k - 1
    cnt = 0
    while j >= 0 and _is_amount_cell(cells[j]):
        cnt += 1; j -= 1
    start = k - cnt
    return (start, cnt)

# [ADD] — chuẩn hoá một dòng generic: ghép text trái + giữ tất cả tail numeric
def _project_row_generic(cells: List[str]) -> List[str]:
    if not cells: return []
    # tách hai số dính trong cell trước khi xét tail
    cells = _split_two_amounts_in_cell(cells[:])

    t0, m = _tail_numeric_span(cells)
    left = [c.strip() for c in cells[:t0]]
    right = [normalize_vn_amount(c) for c in cells[t0:]] if m else []

    # ghép text trái (hạn chế cột text bị xé nhỏ)
    ...
    # loại "" ở phần số (nhưng giữ thứ tự/độ dài)
    right = [r for r in right]

    return left + right

# [ADD] — repack generic toàn bảng (không ép 5 cột)
def _repack_generic_table(pipe_text: str) -> str:
    """
    Không giả định CODE/NAME/NOTE. Coi mọi hàng:
      [text-left*] + [numeric-tail*]
    Giữ *toàn bộ* cột số ở rìa phải, hợp nhất mọi cột text bên trái.
    """
    ...
    body   = [[c.strip() for c in ln.split("|")] for ln in lines[1:]]

    proj = [_project_row_generic(r) for r in body]
    K = max(len(r) for r in proj) if proj else 0
    if K <= 0: 
        return pipe_text

    # tạo header “an toàn”
    hdr = [f"C{i+1}" for i in range(K)]
    out = [" | ".join(hdr)]
    for r in proj:
        r2 = r + [""] * (K - len(r))
        out.append(" | ".join(x if x != "" else NULL_TOKEN for x in r2))
    return "\n".join(out)
...
# ---- Helpers: siết hàng trước khi repack (loại cặp CODE|NAME lặp)
def _looks_like_code_name_pair(s_code: str, s_name: str) -> bool:
    """Heuristic: code đúng định dạng + name dài tối thiểu → coi là 1 cặp hợp lệ."""
    return bool(re.fullmatch(r"\d{3}(?:\.\d+)?", (s_code or "").strip())) and len((s_name or "").strip()) >= 3

def _collapse_duplicated_leading_pair(pipe_text: str) -> str:
    """
    Dòng có pattern: CODE | NAME | CODE NAME (lặp) | ...  -> bỏ cặp thứ hai.
    Áp dụng trước repack để tránh trùng cột trái.
    """
    if not pipe_text or not pipe_text.strip():
        return pipe_text
    ...
    out = [lines[0]]  # giữ header
    for ln in lines[1:]:
        parts = [p.strip() for p in ln.split("|")]
        if len(parts) >= 4:
            c1, n1, c2n = _norm(parts[0]), _norm(parts[1]), _norm(parts[2])
            # CODE NAME dính chung ở cột 3?
            m = re.match(r"^\(?\s*(\d{3}(?:\.\d+)?)\)?\s+(.+)$", c2n)
            if m and re.fullmatch(r"\d{3}(?:\.\d+)?", c1) and len(n1) >= 3:
                c2, n2 = _norm(m.group(1)), _norm(m.group(2))
                # so khớp “nới”: n2 == n1 hoặc bắt đầu bằng n1
                if c2 == c1 and (n2 == n1 or n2.startswith(n1)):
                    parts.pop(2)
...
def _repack_pipe_by_inferred_schema(pipe_text: str,
                                    *,
                                    ...
                                    force_five_when_fin_like: bool = True,
                                    keep_group_rows: bool = True,
                                    **_ignored_kwargs) -> str:
    """
    Đóng gói 5 cột: CODE | NAME | Thuyết minh | Số cuối năm | Số đầu năm
    - Dựa trên pipe_to_flex_schema(...)
    - Chỉ NOTE dùng 'None' khi trống (nếu fill_note_with_none=True)
    - END/BEGIN để rỗng nếu không có
    - Tự tách 'CODE NAME...' nếu bị dính
    - Bỏ dòng rác; giữ dòng nhóm theo keep_group_rows
    """
    if not pipe_text or not pipe_text.strip():
        return pipe_text
    ...
    looks_fin = ("mã số" in s0 or "ma so" in s0) or \
                ("thuyết minh" in s0 or "thuyet minh" in s0) or \
                ("số cuối năm" in s0 or "so cuoi nam" in s0) or \
                ("số đầu năm" in s0 or "so dau nam" in s0)
    ...
    def _has_amount(r): 
        return bool((r.get("END") or "").strip() or (r.get("BEGIN") or "").strip())
    ...
    for r in rows:
        ...
        if not code and name:
            m = re.match(r"^\s*(\d{3}(?:\.\d+)?)\s+(.*)$", name)
            if m: code, name = m.group(1).strip(), m.group(2).strip()

        m2 = re.match(r"^\s*(\d{3}(?:\.\d+)?)\s+(.*)$", code)
        if m2:
            tail = m2.group(2).strip()
            if (not name) or name.startswith(tail) or tail.startswith(name):
                code, name = m2.group(1).strip(), (name or tail)

        if not (code or name or note or endv or begv):
            continue
        if not _has_amount(r) and not keep_group_rows:
            continue
...
def process_page(
    ...
    if rois and not force_table:
        ...
        block_text = chosen_pipe or ""  
        block_text = _trim_to_table_body(block_text)
        block_text = _prefilter_table_lines(block_text, yaml_table)

        # (B1) cấy cột NOTE rỗng nếu bảng có CODE/NAME + cột tiền nhưng không có NOTE riêng
        block_text = _auto_insert_virtual_note_col(block_text)

        # (B2) bóc NOTE nằm ở cuối tên (nếu có) -> đẩy vào cột NOTE vừa/đã có
        block_text = _extract_inline_note_and_insert_col(block_text)

        # (B3) gộp cặp CODE|NAME bị lặp ở đầu dòng
        block_text = _collapse_duplicated_leading_pair(block_text)

        # (B4) chuẩn hoá số (chỉ 2 cột tiền phải nhất)
        block_text = _normalize_all_amounts_in_pipe(block_text)

        # (B5) repack về 5 cột, chỉ NOTE mới dùng None
        block_text = _repack_pipe_by_inferred_schema(
            block_text,
            fill_note_with_none=True,
            force_five_when_fin_like=True
        )

        # [ADD] (B6) FALLBACK GENERIC: nếu không phải header VAS thì repack generic
        if not _looks_like_vas_5col(block_text):
            block_text = _repack_generic_table(block_text)
