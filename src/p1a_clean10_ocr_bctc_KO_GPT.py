...
def process_one_page(out_root: str, base: str, page_no: int,
    ...
    # [ADD] --- Áp dụng YAML autofix trước khi gửi GPT ---
    def apply_yaml_autofix(text, yaml_path):
        """Đọc quy tắc autofix từ YAML và áp dụng tự động (ví dụ: sửa 7.107→1.107)."""
        try:
            with open(yaml_path, "r", encoding="utf-8") as f:
                ycfg = yaml.safe_load(f)
        except Exception as e:
            print(f"⚠️ Không thể đọc YAML autofix: {e}")
            return text

        fixes = ycfg.get("autofix", [])
        for rule in fixes:
            cond = rule.get("when", "")
            action = rule.get("do", "")
            ...
            if m1 and m2:
                old_pref, (old_digit, new_digit) = m1.group(1), m2.groups()
                # chỉ sửa nếu có chuỗi end giống prefix
                if old_pref in text:
                    text = text.replace(f"{old_digit}.", f"{new_digit}.", 1)
                    print(f"🔧 YAML autofix: thay '{old_digit}.' → '{new_digit}.' (theo quy tắc {rule})")

            # ---- TH2: map mã sai, ví dụ raw_code 161.2 -> 151.2 ----
            if "raw_code" in cond and "alias(" in action:
                alias_match = re.search(r"alias\('([^']+)'\)", action)
                if alias_match:
                    old_code = alias_match.group(1)
                    if old_code in text:
                        text = text.replace(old_code, rule.get("code", old_code))
                        print(f"🔧 YAML alias fix: {old_code} → {rule.get('code')}")

        return text

    # Đường dẫn YAML hiện hành
    YAML_PATH = r"D:\1.TLAT\3. ChatBot_project\1_Insurance_Strategy\src\p1c_clean10_ocr_bctc.yaml"
    txt = apply_yaml_autofix(txt, YAML_PATH)


    # [ADD] nếu Dùng GPT để viết lại bảng mô tả rõ ràng hơn
