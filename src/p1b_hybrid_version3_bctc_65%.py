...
def clean_number(raw: Optional[str], cfg: ParseConfig) -> Optional[int]:
    ...
    # drop common currency/space chars
    for ch in cfg.drop_chars:
        s = s.replace(ch, "")

    # if many separators, keep only digits and minus
    if s.count(".") + s.count(",") >= 2:
        s = re.sub(r"[^0-9\-]", "", s)
    else:
        s = re.sub(r"[^0-9\.\-]", "", s)

    # VN style: dot as thousand sep
...
def parse_ascii_table(text: str, cfg: ParseConfig) -> List[Dict[str, Optional[str]]]:
    ...
    for ln in text.splitlines():
        ...
        # need at least 6 parts: "", code, label, note, end, begin, ""
        if len(parts) < 6:
            continue
        ...
        if low in {"mã số", "ma so", "ms"}:
            continue
        if set(code_cell) <= set("-+"):
            continue

        if not code_cell:
            # continuation line: append to previous label
            if rows:
                prev_label = rows[-1].get("label") or ""
                rows[-1]["label"] = (prev_label + " " + label_cell).strip()
            continue
...
def split_line_to_row(line: str, cfg: ParseConfig) -> Optional[Dict[str, Optional[str]]]:
    ...
    nums = _NUMBER_CAPTURE.findall(rest)
